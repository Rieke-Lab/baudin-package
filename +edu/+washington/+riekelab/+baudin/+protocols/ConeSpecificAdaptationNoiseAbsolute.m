classdef ConeSpecificAdaptationNoiseAbsolute < edu.washington.riekelab.protocols.RiekeLabProtocol
    ...
    methods
        ...
        function prepareRun(obj)
            ...
            if numel(obj.rig.getDeviceNames('Amp')) < 2
                ...
                if ~strcmp(obj.onlineAnalysis, 'none')
                    obj.analysisFigure = obj.showFigure('symphonyui.builtin.figures.CustomFigure', @obj.updateAnalysisFigure);
        ...
        function createAnalysisFigure(obj)
            ...
            epochTypeKeys = { ...
                sprintf('%s: %i, %s: %i, %s isol.', obj.coneOnConstantToStimulate, obj.constantConeBackground, obj.coneForChangingBackgrounds, obj.changingConeBackgrounds(1), obj.coneOnConstantToStimulate), ...
                sprintf('%s: %i, %s: %i, %s isol.', obj.coneOnConstantToStimulate, obj.constantConeBackground, obj.coneForChangingBackgrounds, obj.changingConeBackgrounds(1), obj.coneForChangingBackgrounds), ...
                sprintf('%s: %i, %s: %i, %s isol.', obj.coneOnConstantToStimulate, obj.constantConeBackground, obj.coneForChangingBackgrounds, obj.changingConeBackgrounds(2), obj.coneOnConstantToStimulate), ...
                sprintf('%s: %i, %s: %i, %s isol.', obj.coneOnConstantToStimulate, obj.constantConeBackground, obj.coneForChangingBackgrounds, obj.changingConeBackgrounds(2), obj.coneForChangingBackgrounds)};
        ...
        function updateAnalysisFigure(obj, ~, epoch)
            ...
            epochTypeKey = sprintf('%s: %i, %s: %i, %s isol.', ...
