classdef LedPulseFamilyWithNegative < edu.washington.riekelab.protocols.RiekeLabProtocol
    ...
    properties
        ...
        includePossibleNegatives = true % Boolean for whether or not possible negative amplitudes are included
        pulsesInFamily = uint16(3)      % Number of pulses in family
        scaleFactor = 2                 % factor by which to scale the amplitudes for successive pulses
    ...
    methods
        ...
        function currAmplitude = amplitudeForPulseNum(obj, pulseNum)
            currAmplitude = obj.amplitudes(pulseNum);
        end
        
        function prepareEpoch(obj, epoch)
            prepareEpoch@edu.washington.riekelab.protocols.RiekeLabProtocol(obj, epoch);
            
            pulseNum = mod(obj.numEpochsPrepared - 1, obj.numAmplitudes) + 1;
