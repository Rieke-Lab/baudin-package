classdef SpatialAdaptationTransfer < edu.washington.riekelab.protocols.RiekeLabStageProtocol
    ...
    methods
        ...
        function p = createPresentation(obj)
    	    ...
    	    index = mod(obj.numEpochsCompleted, 3 + 2*length(obj.variableFlashTime)) + 1;
            ...
            function m = getGrateMean(obj, time)
                ...
                for flash = 1:length(flashTime)
                    if (time > flashTime(flash)/1e3 & time < (flashTime(flash)/1e3 + obj.flashDuration/1e3))
                        m = 2*obj.backgroundIntensity * (1 + 1/(1-obj.testContrast));
                    end
                end
                if ((m*(1+obj.testContrast) + 2*obj.backgroundIntensity * (1 + 1/(1-obj.stepContrast))) > 2)
            ...
            function m = getBackgroundMean(obj, time)
                ...
                if (time > obj.preTime/1e3 & time < (obj.preTime/1e3 + obj.stimTime/1e3))
                    m = 2*obj.backgroundIntensity * (1 + 1/(1-obj.stepContrast));
                end
            end
        ...
        function prepareEpoch(obj, epoch)
            ...
            index = floor(obj.numEpochsCompleted / (3 + 2 * length(obj.variableFlashTime)));
            ...
            index = (index-1)*(3 + 2 * length(obj.variableFlashTime)) + mod(obj.numEpochsCompleted, 3 + 2*length(obj.variableFlashTime)) + 1;
